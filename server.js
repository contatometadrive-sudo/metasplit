...
app.get('/api/campaigns/:id/generate', (req, res) => {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title> </title></head><body><script>!function(){var D=${destsJson},T='${serverUrl}/api/track/${campaignId}',K='ms_uid',CID='${campaignId}',EXP=2592000000,p={};location.search.slice(1).split('&').forEach(function(s){if(!s)return;var i=s.indexOf('=');try{p[decodeURIComponent(i<0?s:s.slice(0,i))]=decodeURIComponent(i<0?'':s.slice(i+1))}catch(e){}});function genUid(){try{if(crypto&&crypto.randomUUID)return crypto.randomUUID();}catch(e){}return Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,11);}var uid=null;try{var raw=localStorage.getItem(K);if(raw){var bar=raw.indexOf('|');if(bar>0){var usv=raw.slice(0,bar),ex=parseInt(raw.slice(bar+1),10);if(usv&&ex>Date.now())uid=usv;}}}catch(e){}if(!uid){uid=genUid();try{localStorage.setItem(K,uid+'|'+(Date.now()+EXP));}catch(e){}}function h01(s){var h=2166136261;for(var i=0;i<s.length;i++){h=Math.imul((h^s.charCodeAt(i))>>>0,16777619)>>>0;}return h/4294967296;}function wpick(D,r){var t=D.reduce(function(s,d){return s+d.weight},0),v=r*t;for(var i=0;i<D.length-1;i++){if((v-=D[i].weight)<0)return D[i];}return D[D.length-1];}var c=wpick(D,h01(uid+':'+CID));var ab=String.fromCharCode(65+Math.max(0,D.indexOf(c)));var eq=c.src.indexOf('='),pk=eq>=0?c.src.slice(0,eq):'src',pv=eq>=0?c.src.slice(eq+1):c.src,U=c.url;try{var u=new URL(c.url);for(var k in p){if(k!==pk&&k!=='ab')u.searchParams.set(k,p[k])}u.searchParams.set(pk,pv);u.searchParams.set('ab',ab);U=u.toString()}catch(e){}window.location.replace(U);try{fetch(T,{method:'POST',headers:{'Content-Type':'application/json'},keepalive:!0,body:JSON.stringify({destination_id:c.id,params:p,referrer:document.referrer||''})})}catch(e){}}();<\/script></body></html>`;
